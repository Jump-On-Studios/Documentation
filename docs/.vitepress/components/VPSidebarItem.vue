...
function title() {
  if (isDark.value && props.item.iconDark) {
    return `<img src="${props.item.iconDark}" /> ${props.item.text}`
  }
  if (props.item.icon) {
    return `<img src="${props.item.icon}" /> ${props.item.text}`
  }
  ...
  <component :is="sectionTag" class="VPSidebarItem" :class="classes">
    <div v-if="item.text" class="item" :role="itemRole" v-on="item.items
      ...
      <VPLink v-if="item.link" :tag="linkTag" class="link" :href="item.link" :rel="item.rel" :target="item.target">
        <component :is="textTag" class="text" v-html="title()" />
      </VPLink>
      <component v-else :is="textTag" class="text" v-html="title()" />

      <div v-if="item.collapsed != null && item.items && item.items.length" class="caret" role="button" aria-label="toggle section" @click="onCaretClick" @keydown.enter="onCaretClick" tabindex="0">
